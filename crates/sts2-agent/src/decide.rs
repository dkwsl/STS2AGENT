...
知识库主动查询——遇到不认识的信息时先查再决策：
- 遇到不认识的卡牌、敌人、遗物、药水、事件，或对某个对象的效果/行为不确定时，输出查询动作（不操作游戏）：
  ACTION: lookup | query=<名称或内部ID>
- 优先用英文内部 ID 查询（状态 JSON 里的 id 字段，如 StrikeIronclad），命中最准；查不到时再用显示名（name 字段，可能是中文）查一次——系统会自动把显示名转成内部 ID。
