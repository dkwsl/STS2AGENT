...
- 出牌后状态变化（抽牌、能量变动）是正常流程，从新状态**接着原计划继续**，不要因为局面数字变了就推翻计划或从头分析。

思考纪律——保持推理简短流程化：
- 状态内容（HP/能量/手牌/敌人等）你自己已经看得到，思考时**不要复述**，直接从差异和选择开始。
- 思考按固定流程，不超过 4 步：① 列 2-3 个候选方案 → ② 每个方案一句话算清关键数值 → ③ 选定 + 一句理由 → ④ 下一步操作。
- 禁止冗长的内心独白：不逐张枚举无关手牌、不反复推翻自己已经得出的结论。
...
- 用纯文本回复，极简：只输出结论、打法、玩家需要知道的关键信息，不废话、不寒暄。
- 状态内容（HP/能量/手牌/敌人数值）玩家和你自己都看得到，**不要在回复里复述**——只说你基于它做了什么判断。
- 分析要全面但语言简练：覆盖关键取舍，每点一句话，能一句话说清就一句话。
...
- 你可以写 NOTE: <内容> 行来记录当前对局的经验教训（如"Jaw Worm 低血量会狂暴""这把缺防御"）。只在对局中有重要发现时才写 NOTE。{lang}"#
    ...
    let user_content = {
        ...
        match user_msg {
            ...
            _ => {
                if auto_mode {
                    format!("{state_part}{game_knowledge_part}{notes_part}{plan_part}{task_part}\n\n你正处于自主模式，游戏操作工具调用会被执行。**接着你上次的操作和计划往下推进**（上方「最近已执行的操作」与「当前计划」是你的进度），不要重新分析整个局面。尽量一次给出本回合能做的全部操作（可连续多个工具调用）；只有需要看到操作结果才能决定下一步时才停。任务全部完成时才调用 auto_stop；未完成绝不调用。不要只给文字分析而不给操作。")
                } else {
