...
impl LlmClient {
    ...
    pub fn from_config(config: &sts2_core::ModelConfig) -> Self {
        Self::new(&config.endpoint, &config.api_key, &config.model)
    }
    ...
    pub fn chat_stream(
        ...
        tokio::spawn(async move {
            ...
            // 没有超时会永远挂起。120 秒零输出视为流结束（软结束，保留已收内容）。
            loop {
                let chunk = match tokio::time::timeout(IDLE_TIMEOUT, stream.next()).await {
                    Ok(Some(c)) => c,
                    Ok(None) => break, // 流正常关闭
                    Err(_) => {
                        // 空闲超时：视为流结束（保留已收到的思考/内容，循环外统一收尾）。
                        // 不发 Error——长思考/服务端间歇无 chunk 时误杀会丢掉整段推理。
                        break;
                    }
                };
